...
public class BasicArticleRepository implements ArticleRepository {
    ...
    private static long articleIdFactory = 0;  //Factory를 여기로 -> (Fatory가 적당한가?! 확인 후 수정 예정) //AtomicLong 쓸 수도 있었을 듯
    ...
    public Optional<Article> getArticleById(Long id) {
        return articleList.stream().filter(article -> article.getId() == id).findAny();  // .Nullable은 없는데 Optional을 만드는 듯(?)
    }

    public void clearStore(){}    //없으면 빨간줄(인터페이스 구조의 단점.........)
}
