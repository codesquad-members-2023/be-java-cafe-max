...
class PostServiceTest {
    ...
    private static Post getTestPost(User user) {
        return new Post.Builder().user(user)
                .id(TEST_ID)
    ...
    void findByIdForEditFormFailedByAuth() {
        ...
        given(post.getId()).willReturn(TEST_ID);
        given(postRepository.findByIdAndIsDeleted(post.getId(), false)).willReturn(Optional.of(post));
        doThrow(UnauthorizedAccessException.class).when(post).checkPermission(anyLong());

        assertThatThrownBy(() -> postService.findByIdForEditForm(post.getId(), user.getId())).isInstanceOf(
    ...
    void getAllSimplePostForm() {
        ...
        assertThat(allSimplePostForm).usingRecursiveComparison()
                .isEqualTo(SimplePostForm.toSimplePostForm(List.of(post)));
    ...
    void deleteFailedByNotExistsId() {
        ...
        assertThatThrownBy(() -> postService.delete(post.getId(), user.getId())).isInstanceOf(
    ...
    public void getAllSimplePostFormByUser() {
        ...
        assertThat(allSimplePostFormByUser).usingRecursiveComparison()
                .isEqualTo(SimplePostForm.toSimplePostForm(List.of(post)));
    ...
    public void getPagesInfoByUser() {
        ...
        assertThat(pagesInfoByUser).usingRecursiveComparison()
                .isEqualTo(PagesInfo.of(CURRENT_PAGE, getPages(TOTAL_PAGES)));
    }
