...
class PostControllerTest {
    ...
    class PostPage {
        ...
        void viewPostPageSuccess() throws Exception {
            mockMvc.perform(get("/posts/form").session(session)).andExpect(status().isOk());
        }
    ...
    class PostAddBottomTest {
        ...
        void addPostSuccess() throws Exception {
            ...
            mockMvc.perform(post("/posts").param(TITLE, TEST_TITLE).param(TEXT_CONTENT, TEST_CONTENT).session(session))
                    .andExpect(status().is3xxRedirection())
        ...
        void addPostFailureFailedByType(String title, String textContent) throws Exception {
            mockMvc.perform(post("/posts").param(TITLE, title).param(TEXT_CONTENT, textContent).session(session))
        ...
        void addPostFailureFailedByNotUser() throws Exception {
            mockMvc.perform(post("/posts").param(TITLE, TEST_TITLE).param(TEXT_CONTENT, TEST_CONTENT))
                    .andExpect(status().is3xxRedirection())
                    .andExpect(redirectedUrl("/users/login"));
        }
    }
    ...
    class PostPageTest {
        ...
        class OpenTest {
            ...
            void viewPostPageFailedByPageId() throws Exception {
                ...
                mockMvc.perform(get("/posts/" + NOT_EXISTING_POST_ID).session(session))
        ...
        class EditTest {
            ...
            void viewPostEditPageSuccess() throws Exception {
                ...
                mockMvc.perform(get("/posts/" + post.getId() + "/editForm").session(session))
                        .andExpect(status().isOk())
            ...
            void viewPostEditPageFailedByOtherUser() throws Exception {
                Post post = mock(Post.class);
                given(post.getId()).willReturn(1L);
                given(postService.findByIdForEditForm(anyLong(), anyLong())).willThrow(
            ...
            void editPostSuccess() throws Exception {
                ...
                mockMvc.perform(put("/posts/" + post.getId()).param(TITLE, EDIT_TITLE)
                        .param(TEXT_CONTENT, EDIT_TEXT_CONTENT)
                        .session(session)).andExpect(status().isOk()).andExpect(view().name("post/detail"));
            }
            ...
            void editPostFailedByType(String targetTitle, String targetContent) throws Exception {
                ...
                mockMvc.perform(put("/posts/" + post.getId()).param(TITLE, targetTitle)
            ...
            void editPostFailedByPostId() throws Exception {
                given(postService.updateFromPostForm(anyLong(), any(), anyLong())).willThrow(
                        UnauthorizedAccessException.class);

                mockMvc.perform(put("/posts/" + NOT_EXISTING_POST_ID).param(TITLE, EDIT_TITLE)
                        .param(TEXT_CONTENT, EDIT_TEXT_CONTENT)
                        .session(session)).andExpect(status().is4xxClientError()).andExpect(view().name("error/4xx"));
            }
            ...
            void deletePostFailedByOtherComments() throws Exception {
                ...
                mockMvc.perform(delete("/posts/" + post.getId()).session(session))
                        .andExpect(status().is4xxClientError())
                        .andExpect(view().name("post/editForm"));
            }
            ...
            void deletePostFailedByAuth() throws Exception {
                Post post = mock(Post.class);
                given(post.getId()).willReturn(1L);
                doThrow(UnauthorizedAccessException.class).when(postService).delete(anyLong(), anyLong());
