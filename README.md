...
- [x] 회원 목록을 조회한다.
- [x] 이메일, 패스워드,닉네임을 입력 받아서 회원 가입을 한다.
  - [x] 회원 가입 후 회원 목록 조회로 이동한다.
- [x] 회원 프로필을 조회한다.
- [x] 회원 가입된 회원의 프로필을 수정한다.
  - [x] 프로필 수정 후 해당 회원을 프로필 조회로 이동한다.
...
- [x] 게시글 목록을 조회한다.
- [x] 제목, 내용을 입력 받아서 게시글을 작성한다. 
  - [x] 게시글 작성 후 게시글 목록으로 이동한다. 
- [x] 게시글을 상세 조회한다. 
...
- [x] 회원 저장, 조회, 수정 JDBC를 이용하여 DB에서  데이터를 관리한다.
- [x] 게시글 저장, 조회  JDBC를 이용하여 DB에서  데이터를 관리한다.
- [x] 게시글 조회시 조회수 증가한다.
- [x] 회원 프로필 수정시 기존 비밀번호를 확인한다.
- [x] Controller, Service, Repository에 대하여 테스트 코드를 작성한다.
- [x] 배포 스크립트 작성한다.
- [x] aws를 이용하여 배포를 한다.
...
- [x] 이메일, 패스워드를 입력하여 로그인을 한다.
  - [x] 로그인 상태인 경우 상단 메뉴에 로그아웃, 개인정보 수정이 표시되어야 한다.
- [x] 로그인이 된 상태에서 로그아웃을 한다.
  - [x] 로그인 상태가 아닌 경우에는 로그인, 회원가입이 표시되어야 한다.
...
- [x] 로그인한 사용자만 게시글을 작성한다.
- [x] 로그인한 사용자만 게시글을 세부내용을 조회한다.
- [x] 게시글을 수정한다.
  - [x] 작성자와 로그인한 회원이 같은 경우에만 가능하다.
  - [x] 작성자가 아닌 경우 에러페이지로 이동한다.
- [x] 게시글을 삭제한다.
  - [x] 작성자와 로그인한 회원이 같은 경우에만 가능하다.
  - [x] 작성자가 아닌 경우 에러페이지로 이동한다.
...
- [x] 로그인한 사용자만 게시글에 댓글을 추가할 수 있다.
- [x] 게시글 삭제시 데이터의 상태를 삭제 상태로 변경한다.
  - [x] 댓글이 없는 경우 삭제가 가능하다.
  - [x] 게시글 작성자와 댓글 작성자가 다를 경우 삭제는 불가능하다.
  - [x] 게시글 작성자와 댓글 작성자가 모두 같은 경우는 삭제가 가능하다.
- [x] 댓글 삭제도 데이터의 상태를 삭제 상태로 변경한다.
  - [x] 로그인 사용자와 댓글 작성자가 같은 경우에만 댓글을 삭제할 수 있다.
...
- [x] 댓글 기능들을 AJAX와 RestController로 구현한다.
- [x] 데이터베이스는 MySQL.8.0으로 변경한다.
