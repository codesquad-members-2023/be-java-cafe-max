...
- [x] 로그인 한 사용자는 자신의 글을 수정 및 삭제할 수 있다.
  - [x] 수정/삭제 기능은 로그인 사용자와 글쓴이의 사용자 아이디가 같은 경우에만 가능하다.
  - [x] 상황에 따라 "다른 사람의 글을 수정할 수 없다."와 같은 에러 메세지를 출력하는 페이지로 이동하도록 구현한다.

<br>

### 프로그래밍 요구사항
- [x] 글쓴이 값은 User의 name 값을 가지는 것으로 구현한다.
...
- [x] 게시글 삭제하기는 `@DeleteMapping`을 사용해 매핑한다.
...
### 기능요구사항
- [x] 로그인한 사용자는 게시글 상세보기 화면에서 댓글을 볼 수 있다.
...
- [ ] AWS EC2로 배포를 진행하고 README에 배포 URL을 기술한다.
...
- [ ] 게시글을 삭제할 때 댓글 또한 삭제해야 하며, 댓글의 삭제 또한 삭제 상태(deleted)로 변경한다.
...
### 기능요구사항
- [ ] 질문에 답변을 추가할 때 화면 전체를 업데이트 하지 않고 답변을 추가한다.

<br>

### 프로그래밍 요구사항
- [ ] RestController와 AJAX로 구현한다.
- [ ] 데이터베이스는 MySQL 8.0을 이용한다.
- [ ] AWS EC2로 배포한다. RDS는 사용하지 않는다.
- [ ] 배포 IP를 README에 기술한다.
